...
process make_sv_beds {
       ...
       Rscript --exclude=fsupeksvr5 !{baseDir}/vcf_to_bed.R --VCF !{sample}.sv.filt.vcf.gz --close !{params.close_value} --closer !{params.closer_value}
...
process make_vcfs {
    ...
    bcftools view -f PASS --types snps --regions-file unclustered.bed !{sample}.filt.vcf.gz |  bcftools norm -d all -f !{fasta_ref} | sort -k1,1V -k2,2n | bgzip -c > !{sample}_unclustered.snv.vcf.gz
